  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Vehicle Type</h1>
          <div className="text-sm text-gray-500">
            Dashboard <span className="text-orange-500">/ Vehicle Type</span>
          </div>
        </div>
        <button
          onClick={onAdd}
  ...
  async function handleSave() {
    if (!form.name.trim()) {
      Swal.fire({
        ...
        text: "Please enter Vehicle Type name.",
      });
      return;
    }
...
export default function VehicleType() {
  ...
  const pageCount = Math.max(1, Math.ceil(type.length / pageSize));
  const current = type.slice((page - 1) * pageSize, page * pageSize);
